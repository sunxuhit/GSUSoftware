...
PhEventPlaneMaker::PhEventPlaneMaker(const char*outputfile)
{
  mRC = NULL;
  // RC flags
  mRunSelection    = 0;
...
PhEventPlaneMaker::~PhEventPlaneMaker()
{
  delete mBbcCalib;
  delete mBbcGeo;
...
int PhEventPlaneMaker::Init(PHCompositeNode *topNode) 
{
  cout << "PhEventPlaneMaker::Init => " << endl;
...
int PhEventPlaneMaker::InitRun(PHCompositeNode *topNode) 
{
  cout << "PhEventPlaneMaker::InitRun => " << endl;
  ...
  {
    cout << "isGoodRun => could not find in good run list! & drop the run in PhEventPlaneMaker!" << endl;
    return ABORTRUN;
  }
  ...
  if(mRunIndex < 0)
  {
    cout << "find_runIndex => could not find in good run list! & drop the run in PhEventPlaneMaker!" << endl;
    return ABORTRUN;
  }
  else
  {
    cout << "PhEventPlaneMaker -> runId: " << mRunId << " => runIndex: " << mRunIndex << endl;
  }
...
int PhEventPlaneMaker::process_event(PHCompositeNode *topNode) 
{
...
int PhEventPlaneMaker::End(PHCompositeNode *topNode) 
{
  File_mOutPut->cd();
...
int PhEventPlaneMaker::getNodes(PHCompositeNode *topNode)
{
...
bool PhEventPlaneMaker::isMinimumBias()
{
...
bool PhEventPlaneMaker::isMinimumBiasNarrowVtx()
{
  // Au+Au @ Run14 trigger selection
  // const unsigned int BBCLL1_narrowvtx = 0x00000010; // 4 - BBCLL1(>1 tubes) narrowvtx
  // const unsigned int BBCLL1           = 0x00000020; // 5 - BBCLL1(>1 tubes)
