...
void Run_PhEventPlaneMakerAuAuRun14(const char *outFile = "BbcEventPlane.root")
{
  ...
  gSystem->Load("/direct/phenix+u/xusun/WorkSpace/PHENIX/install/lib/libPhEventPlaneMakerAuAuRun14.so");
  ...
  // rc->set_IntFlag("ANA_MODE",3); // produce di-muon TTree
  rc->set_IntFlag("QA_BBC",0);
  rc->set_IntFlag("DEBUG_BBC",0);


  PhEventPlaneMaker *ana = new PhEventPlaneMaker(outFile);
