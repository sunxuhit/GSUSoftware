...
void RecoEPProManager::fillPro_BbcResolution(float Psi_BbcSouth, float Psi_BbcNorth, int order, int runIndex, int cent20)
...
void RecoEPProManager::initPro_FvtxResolution()
{
  ...
  for(int i_order = 0; i_order < 3; ++i_order)
  {
    std::string ProName = Form("p_mResolution_FvtxSub_%s",Order[i_order].c_str());
    p_mResolution_FvtxSub[i_order] = new TProfile(ProName.c_str(),ProName.c_str(),vecMesonFlow::mNumOfCentrality20,-0.5,19.5);
...
void RecoEPProManager::fillPro_FvtxResolution(float Psi_FvtxSouth, float Psi_FvtxNorth, int order, int runIndex, int cent20)
{ // PsiShift_FvtxNorth - PsiShift_FvtxSouth
  float harmonic = vecMesonFlow::mHarmonic[order];
  float resolution = TMath::Cos(harmonic*(Psi_FvtxNorth-Psi_FvtxSouth));
  p_mResolution_FvtxSub[order]->Fill((double)cent20,(double)resolution);
